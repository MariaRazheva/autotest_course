...
def repeats(our_str):
    """
    Возвращает строку, где рядом с каждым символом через нижнее подчеркивание указано числом, сколько раз данный символ
    уже встречался в строке
    :param our_str: исходная строка
    :return: строка, где рядом с каждым символом через нижнее подчеркивание указано числом, сколько раз данный символ
    уже встречался в строке
    """
    list_new_str = list()
    new_str = ''
    ...
    for i in range(len(our_str)):
        ...
        new_str += str(letters_dict[our_str[i]])
    return new_str
