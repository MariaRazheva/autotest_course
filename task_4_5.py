...
def josephus_task(num_people, kill_num):
    """
    Получить решение задачи Иосифа Флавия
    :param num_people: количество человек
    :param kill_num: номер выбывающего по счёту
    :return: номер последнего оставшегося
    """
    lst_of_people = [i for i in range(num_people)]
    k = 0
    while len(lst_of_people) > 1:
        for i in range(kill_num):
            if (i != 0): k += 1
            if k == len(lst_of_people):
                k = 0
        lst_of_people.pop(k)
    survivor = lst_of_people[0] + 1
    return survivor
