...
def everything_for_your_cat(cats_data):
    """
    Получает из строк формата <кличка>, <возраст>, <имя>, <фамилия> строку, где записи имеют формат <имя>, <фамилия>: <питомец_1>,
    <возраст_1>; <питомец_2> и т.д., и записи разделены переводом на новую строчку
    :param cats_data: список строк указанного выше формата
    :return: строка с записями указанного формата и разделёнными переходами строки
    """
    set_owners = set()
    list_of_str_cats = []
    our_str = ''
    for i in range(len(cats_data)):
        if not (cats_data[i][2] + ' ' + cats_data[i][3] in set_owners):
            list_of_str_cats.append(
                cats_data[i][2] + ' ' + cats_data[i][3] + ': ' + cats_data[i][0] + ', ' + str(cats_data[i][1]))
        else:
            for j in range(len(list_of_str_cats)):
                if list_of_str_cats[j].startswith(cats_data[i][2] + ' ' + cats_data[i][3]):
                    list_of_str_cats[j] += ('; ' + cats_data[i][0] + ', ' + str(cats_data[i][1]))
        set_owners.add(cats_data[i][2] + ' ' + cats_data[i][3])
    for k in range(len(list_of_str_cats)):
        our_str += (list_of_str_cats[k] + '\n')
    return our_str
